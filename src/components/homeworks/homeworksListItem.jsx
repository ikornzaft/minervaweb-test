...
const HomeworksListItem = ({ homework, key }) => {
  ...
  const date = new Date(homework.inserted.timestamp).toLocaleDateString(
    'es-Es',
    options
  );
      ...
      <Stack width="100%" justifyContent="flex-start">
        ...
        <Stack width="100%" direction="row" alignItems="center" paddingX={4}>
          <Stack alignItems="flex-start" paddingTop={2}>
            ...
            >
              {homework.contentHeader.descriptor.title}
            </Heading>
            <Box textAlign="left" marginTop="0" paddingLeft={0}>
              <Text as="h5" fontSize="sm" fontWeight="400">
                {ParagraphReducer(homework.contentHeader.descriptor.subtitle)}
              </Text>
            </Box>
          </Stack>
        </Stack>
